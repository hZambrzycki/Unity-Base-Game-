...
namespace RPG.UI.Shops
{
    public class RowUI : MonoBehaviour
    {
        ...
        public void Add()
        {   
            if(item.GetQuantityInTransaction() > item.GetAvailability())
            {
                currentShop.AddToTransaction(item.GetInventoryItem(), -1);
            }
            else if (item.GetQuantityInTransaction() == item.GetAvailability())
            {
                currentShop.AddToTransaction(item.GetInventoryItem(), 0);
            }
            else
            {
                currentShop.AddToTransaction(item.GetInventoryItem(), 1);
